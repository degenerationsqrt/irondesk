...
- [ ] Deploy and verify the completed-workout export endpoint and updated privacy/setup pages.
- [ ] Verify optional `WRITE_EXERCISE` permission and complete the round-trip phone checks in
      [Workout export](WORKOUT_EXPORT.md); update Play declarations before Play distribution.
...
- [ ] Confirm `versionName` is `1.2.0-beta.1`, `versionCode` is `12001`, and target/compile SDK are 36.
...
- [ ] Confirm CI metadata checks report package `app.irondesk.health`, version code `12001`, version
      name `1.2.0-beta.1`, and target SDK 36.
...
- [ ] Name direct artifacts unambiguously, for example
      `IronDesk-Health-1.2.0-beta.1-signed.apk`; never reuse the legacy
...
& "$buildTools\aapt.exe" dump badging .\IronDesk-Health-1.2.0-beta.1-signed.apk
& "$buildTools\apksigner.bat" verify --verbose --print-certs .\IronDesk-Health-1.2.0-beta.1-signed.apk
Get-FileHash .\IronDesk-Health-1.2.0-beta.1-signed.apk -Algorithm SHA256
```
