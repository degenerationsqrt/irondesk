...
The Android companion can connect to Health Connect on Android 14 or newer. After the user grants access, IronDesk reads daily summaries for steps, heart rate, resting heart rate, sleep duration, weight, body-fat percentage, total calories burned, exercise duration, and VO2 max. Access is read-only. IronDesk does not request medical-record permissions, exercise routes, raw location, or background health access. Health Connect exercise time is not converted into a detailed workout, avoiding duplicates with Garmin FIT/CSV activity imports.
